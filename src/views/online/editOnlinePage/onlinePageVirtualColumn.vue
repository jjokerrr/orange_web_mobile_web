    ...
    <div class="table-column-list">
      <div class="title">
        ...
        <el-button :size="defaultFormItemSize" type="text"
          ...
          @click.stop="onAddNewVirtualColumn">新增</el-button>
      </div>
      <el-scrollbar :style="{height: height - 51 + 'px'}">
        ...
        >
          ...
          <el-button :size="defaultFormItemSize" type="text" style="margin-left: 10px;"
        ...
        <el-col :span="24" style="border-top: 1px solid #EBEEF5;">
          <el-table :size="defaultFormItemSize" :data="virtualColumnFilterList" :show-header="false" empty-text="请添加过滤条件">
            ...
            <el-table-column label="参数名称">
              <template slot-scope="scope">
                <el-button type="text" style="text-decoration: underline;" @click="onEditColumnFilter(scope.row)">{{(scope.row.column || {}).columnName || '未知字段'}}</el-button>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    refreshData (data) {
      ...
      if (data.path === 'thirdEditVirtualColumnFilter') {
    ...
    loadOnlineVirtualColumnList () {
      ...
      OnlineVirtualColumnController.list(this, params).then(res => {
        ...
        if (this.currentColumn == null && Array.isArray(this.allVirtualColumnList) && this.allVirtualColumnList.length > 0) {
          this.currentColumn = this.allVirtualColumnList[0];
        }
      }).catch(e => {});
    },
  ...
  watch: {
    currentColumn: {
      handler (newValue) {
        if (newValue == null) {
          if (Array.isArray(this.allVirtualColumnList) && this.allVirtualColumnList.length > 0) {
            this.currentColumn = this.allVirtualColumnList[0];
          }
        } else {
