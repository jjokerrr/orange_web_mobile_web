import { SysCustomWidgetType, SysOnlineFormType } from '@/staticDict/index.js';
import { SysOnlineFieldKind } from '@/staticDict/onlineStaticDict.js';
 ...
 */
function columnIsValidByWidgetType (column, widgetType, formType) {
  if (column == null) {
    return {
      disabled: true,
      warningMsg: '错误的字段数据'
  ...
  let columnFieldType = getColumnDataType(column);
  let disabled = false;
  let warningMsg = null;
  if (column.fieldKind === SysOnlineFieldKind.UPLOAD) {
    ...
    disabled = widgetType !== SysCustomWidgetType.Upload && widgetType !== SysCustomWidgetType.Image;
    warningMsg = SysOnlineFieldKind.getValue(column.fieldKind);
  } else if (column.fieldKind === SysOnlineFieldKind.RICH_TEXT) {
    disabled = widgetType !== SysCustomWidgetType.RichEditor;
    warningMsg = SysOnlineFieldKind.getValue(column.fieldKind);
  } else if (column.fieldKind === SysOnlineFieldKind.RICH_TEXT) {
  } else if (
    ...
    disabled = (widgetType !== SysCustomWidgetType.Label &&
      [SysOnlineFormType.QUERY, SysOnlineFormType.ADVANCE_QUERY, SysOnlineFormType.ONE_TO_ONE_QUERY].indexOf(formType) === -1);
    warningMsg = SysOnlineFieldKind.getValue(column.fieldKind);
  } else {
    switch (widgetType) {
      ...
      case SysCustomWidgetType.CitySelect:
        disabled = columnFieldType !== 'String';
        break;
  ...
  if (disabled && warningMsg == null) {
    warningMsg = widgetType === SysCustomWidgetType.DataSelect ? '不是一对一关联字段' : '字段类型不匹配';
  }
